import { RuleConditionFact, FamilyStatusEnum, RuleConditionsInput } from '@server/src/api_docs/api'
...
export function useMatchingApplications(conditions: RuleConditionsInput) {
  ...
  const input = map(_input, (condition: { fact: RuleConditionFact; value: string | boolean }) => {
    if (condition.fact === RuleConditionFact.FamilyStatus) {
      if (condition.value !== String(FamilyStatusEnum.NEW) && condition.value !== String(FamilyStatusEnum.RETURNING)) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await apiClient.ruleEvaluationControllerGetMatchingApplicationsCount(input as unknown as RuleConditionsInput)
      return data
    },
