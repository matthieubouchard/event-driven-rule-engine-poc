...
const origin = process.env.REMIX_DEV_ORIGIN || 'http://localhost:3001/'
...
const BUILD_PATH = path.resolve('../client/build/index.cjs')
const PUBLIC_PATH = path.resolve('../client/public')
const VERSION_PATH = path.resolve('../client/build/version.txt')
...
async function createDevRequestHandler(initialBuild) {
  ...
  async function handleServerUpdate() {
    ...
    broadcastDevReady(build, origin)
  }
...
async function getRemixHandler() {
  ...
  const remixHandler = isDev ? await createDevRequestHandler(remixBuild) : createRequestHandler({ build: remixBuild })

  const handler: RequestHandler = async (req, res, next) => {
    // if the request is for the API, skip Remix and let Nest handle it
    if (req.originalUrl.startsWith('/api') || req.originalUrl.startsWith('/build') || req.originalUrl.startsWith('/assets')) {
      return next()
    }
